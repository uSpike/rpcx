...
class _RequestTask:
    ...
    def force_abort_streams(self) -> None:
        """Force close the producers to raise EndOfStream for any waiting consumers."""
...
class RPCClient:
    ...
    async def request(self, method: str, *args: Any, **kwargs: Any) -> Any:
        if self._closed:
            raise RuntimeError(f"Cannot request {method}, server stream is closed")
